...
Q1: **6 running IoT-related deployment types** are represented in the live `smart-city` namespace: `traffic-camera`, `healthcare-api`, `parking-system`, `env-sensor`, `street-lighting`, and `mqtt-broker` (broker is infrastructure/support, but counted in the 13-device metric). The IDS UI also maps these to real-world device categories (ONVIF camera, FHIR gateway, MQTT/CoAP parking sensors, Modbus/OPC UA environmental sensors, DALI/TALQ lighting, message broker).  
**Code ref:** `services/ids-api/src/api/iot.py:72`, `services/ids-api/src/api/iot.py:137`, `services/ids-api/src/api/_state.py:1227`  
**Verify:** `kubectl get deploy -n smart-city` and `kubectl get pods -n smart-city --field-selector=status.phase=Running | grep -E 'traffic-camera|healthcare-api|parking-system|env-sensor|street-lighting|mqtt-broker' | wc -l` (expected `13`)
...
Q3: Replica counts (live): `traffic-camera=2`, `healthcare-api=2`, `parking-system=2`, `env-sensor=3`, `street-lighting=3`, `mqtt-broker=1` = **13 pods**. It is not 1:1 pod-to-device because each pod emulates multiple logical devices (e.g., parking slots, luminaires, stations) for realism and resource efficiency.  
...
Q4: The dashboard “13 devices” metric is **prefix-based pod discovery**, not label-based. `refresh_iot_active_metric()` counts running pods whose names start with known IoT prefixes; separately, deployments/services use `app=<name>` labels/selectors.  
**Code ref:** `services/ids-api/src/api/_state.py:1201`, `services/ids-api/src/api/_state.py:1212`, `services/ids-api/src/api/_state.py:1227`, `k8s-manifests/services-no-build.yaml:8`, `k8s-manifests/services-no-build.yaml:12`, `k8s-manifests/mqtt-broker.yaml:6`, `k8s-manifests/mqtt-broker.yaml:10`  
**Verify:** `kubectl get pods -n smart-city -l 'app in (traffic-camera,healthcare-api,parking-system,env-sensor,street-lighting,mqtt-broker)' -o name` (expected 13 pods)
...
- The dashboard “13 devices” count is a pod-count metric with a fallback expected count (`13`) in `services/ids-api/src/api/_state.py:1227`, not a strict inventory DB count.
