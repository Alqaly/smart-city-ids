...
# Usage: bash scripts/eval-day.sh [--profile minimal|standard|full] [--runs N]
# =============================================================================
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --help)
            cat <<'HELP'
Usage: eval-day.sh [options]

Options:
  --profile PROFILE   minimal, standard, or full (default: minimal)
  --runs N            Number of evaluation rounds (default: 1)
  --no-bootstrap      Skip readiness check at start
...
if [[ "$NO_BOOTSTRAP" -eq 0 ]]; then
    bash "$SCRIPT_DIR/deploy-code.sh"
    bash "$SCRIPT_DIR/readiness-check.sh" --quick || true
fi
