...
def publish():
    ...
    if emulate_network_conditions():
        ...
        return jsonify({"published": False, "topic": MQTT_TOPIC, "message": message, "error": "Emulated packet loss"})
...
def emulate_network_conditions():
    ...
    if random.random() < SIM_LOSS:
        return False  # Emulate lost packet
    return True

def emulate_device_failure():
    # Emulate device failure
    if random.random() < FAIL_PROB:
        print(f"[FAILURE] {POD_NAME} is emulating a device failure. Stopping MQTT network loop for {FAIL_DUR} seconds.")
